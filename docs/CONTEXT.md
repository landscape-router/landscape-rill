...
> 版本：v0.4（2026-09-03 修订：帧头措辞同步——42B 唯一帧头、path_id=0 = 默认路径（REQ-066））
...
全部已收敛决策（含提出日期与动机）迁移至 [requirements/](./requirements/README.md)（REQ-001~REQ-039、REQ-042、REQ-043 已合并，REQ-040/041/044~050 挂账中）。状态表按提出日期排序，即原决策时间线视图。
...
│  mesh 接入（自研控制面 + 42B 帧数据面）⇄ rill 节点                   │
...
| mesh 模式 | 自建网络：自研控制面（coordinator）+ 自研数据面（42B 帧头，含 path_id） |
| coordinator | mesh 控制面服务器：节点注册、身份绑定签名、key_dst 派生、netmap 权威 |
| rill 节点 | 自研 mesh 协议节点（42B 帧 + 自研控制面），所有节点必具的协议身份 |
...
| 路径服务（Path Service） | v1.5 控制面概念：coordinator 维护路径表（PathMap），节点经 Path\* 消息族请求/接收/撤销候选路径（CONTROL_PLANE §3.11）；节点经 Path\* 消息族维护候选路径（FRAME_HEADER §2.1 path_id 字段） |
...
| path_id | 8B 路径标识（帧头字段 18..26）；`path_id = 0` = 默认路径 = key_dst 语义；纳入 route_mac 与 AEAD AAD（FRAME_HEADER §2.1/§9） |
| key_path | 路径授权密钥 `KDF(主密钥, path_id, path_epoch)`，按路径签发、只发路径参与者；显式路径（path_id≠0）route_mac 用；路径级授权非源认证（CONTROL_PLANE §3.11.5） |
...
| relay（中继） | 42B 帧转发节点；三层模型：coordinator 兜底 + 自愿节点（relay 位 opt-in）+ 独立部署 |
| probe（探测包） | 直连可达性验证专用小包（magic + node_id + nonce），独立于 42B 帧 |
...
推论：官方客户端数据面是**标准 WireGuard**，不认识 42B 帧头——手机永远只能作为"叶子客户端"经 rill ext 节点接入（WG ⇄ rill ext 节点），这是"双数据面"架构的成因。
...
**远期特性（proposed 需求）**：
- **路径服务（v1.5 控制面 / v2 数据面，REQ-034，设计已合并 CONTROL_PLANE §3.11）**：PathMap 与 netmap 分离；PathRequest/PathResponse/PathUpdate/PathWithdraw/PathProbe 消息族；每目标 2~4 候选路径 + flow hash + 快速切换 + 路径生命周期；**帧头固定 8B path_id**（42B 帧头，纳入 route_mac/AAD），显式路径 route_mac 用 `key_path = KDF(主密钥, path_id, path_epoch)` 按路径签发（`path_id=0` = 默认路径 = key_dst）；**路径集合 = 路径级 ACL**（与 ACL v2 衔接）
...
| P1 | mesh 骨架：crate 落地（tun + 用户态转发骨架）+ mesh 接入（单 coordinator + 42B 帧）——**大部完成（REQ-022~REQ-032）** |
| v1.5 | 路径服务（控制面）：Path\* 消息族 + 每目标 2~4 候选路径/快速切换 + 路径生命周期 + flow hash（CONTROL_PLANE §3.11，REQ-034）；数据面帧头 path_id=0 默认路径 |
| P2 | 接入：ts2021 客户端接入（连 headscale + subnet router 广播）+ dn42 接入（boringtun + eBGP-lite） |
| P3 | 融合与自研：路由策略引擎完善 + exit 双向语义 + 自研 ts2021 服务端替换 headscale + Raft + relay 治理与路径可观测（REQ-062/064） |
