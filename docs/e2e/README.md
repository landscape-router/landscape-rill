...
> 场景清单与状态见 [../tests/README.md](../tests/README.md)（验收矩阵）与 [../tests/integration.md](../tests/integration.md)（跨腿联动）。

## 1. 环境方案（仅容器）

- 每节点 = 一个容器（边缘节点实例），NAT 场景用额外容器/iptables 模拟
...
| 脚本 | 覆盖 | 前置条件 |
|---|---|---|
| `e2e/run_e2e.sh` | mesh 腿全链路（双节点 ping/ping6、IPv6 双栈、泛洪） | docker + compose 构建 |
...
| P2 | 接入腿：ts2021 客户端腿（subnet router 广播）、dn42 腿（eBGP-lite 会话） | ⏳ 未开始 |
