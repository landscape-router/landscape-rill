...
> 版本：v0.2（2026-08-31 修订：节点类型命名定稿，新增 §4）｜ 相关需求：REQ-005 / REQ-006 / REQ-019 / REQ-030
...
| 轴 | 名称 | 定义 |
|---|---|---|
| 协议身份 | **rill 节点** | 自研 mesh 协议节点（34B 帧 + 自研控制面），所有节点必具的身份 |
...
- **兼容数据面（WireGuard）**：面向官方客户端（手机/平板）。手机不认识 34B 帧头，只能做"叶子客户端"：`手机 ⇄ rill ext 节点（boringtun WG 端点）`。
- **内网数据面（34B 帧）**：rill 节点之间的内部通道（XDP 友好、中继优化、联邦预留），与官方客户端完全解耦。
