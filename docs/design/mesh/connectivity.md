...
> 版本：v0.7（2026-09-03 修订：帧头措辞同步 42B 唯一帧头（REQ-066））｜ 相关需求：REQ-007 / REQ-014 / REQ-017 / REQ-028 / REQ-046 / REQ-054 / REQ-059

## 1. 问题与目标

默认路径帧（path_id=0）的转发依据是 `to_node_id → UDP 端点` 表，需要三块拼图：
...
### 2.1 数据面端口分派（probe vs 42B 帧）

节点数据面 UDP 端口同时承载 42B 帧与 probe 小包，按**首字节**分派：

- `0x01..=0x0F` → 42B 帧（帧头 `version` 值域，合法值 0x01，FRAME_HEADER §2.1）
...
| 攻击 | 后果 | 防护/边界 |
|---|---|---|
| 伪造 probe-response | 诱导错误的直连决策 | 伤害上限为 DoS；真实流量是 AEAD 封装的 42B 帧，攻击者读不了也注入不了 |
