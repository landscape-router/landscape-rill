...
- **长度边界（已修订 2026-09-03，DN42_LEG §7 ⑤）**：原 IPv4 `< /8`、IPv6 `< /32` 注册层边界**撤销**，白名单覆盖为唯一门——dn42 聚合公告（fd00::/8）属有意宽前缀，宽窄的权威在管理面白名单本身（config 显式声明即意图）；covered-by fail-closed 语义不变
...
- **边界**：长度边界撤销（2026-09-03 修订，见上）；默认出口走 exit 语义（ROUTE_ENGINE §5）
