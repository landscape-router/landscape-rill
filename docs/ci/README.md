...
| `e2e-mesh.yml` | `e2e/run_e2e.sh` 五场景：direct（coord + node-a/b，IPv4+IPv6 ping）、relay（a—b—c 线形经 b 中继，`MESH_E2E_SCENARIO=relay`）、persist（REQ-037）、log（REQ-039）、reload（REQ-038 SIGHUP 重载） | PR + push main + workflow_dispatch | 否 |
