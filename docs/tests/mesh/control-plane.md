...
- [ ] CTL-21：节点遥测上报（REQ-052，状态：`待补充`）——单测：Heartbeat 遥测载荷编解码向后兼容（旧节点空载荷照常）；区间计数语义（上报即清零、下区间不含旧值）；coord 聚合 latest-wins 快照；RegisterRequest.version 空值兼容。e2e：双节点互 ping 后状态端点 per-peer 计数 > 0、直连对非空
