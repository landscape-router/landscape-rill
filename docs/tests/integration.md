...
> 多腿联动的端到端场景——各子系统单测之外，验证"手机/mesh/dn42/tailnet/WAN"之间的真实联动。
...
- 状态：`部分覆盖`
- 证据：e2e/run_e2e.sh
- 缺口：mesh 节点互 ping 已闭环（FRM-10）；**手机侧（官方客户端）接入 mesh 资源未验证**
- 说明：手机访问 mesh 节点 A 背后 LAN：请求到达 A 侧设备，A 回包经回程路由（tailnet 前缀公告）回到手机；双向 ping/HTTP 通
...
- 说明：手机流量 → 边缘节点 → tun0 → WAN NAT → 互联网；回程对称
...
- 说明：手机访问 dn42 前缀：边缘节点引擎裁决 dn42 腿 → boringtun 隧道 → dn42 peer

## E2E-04 mesh 节点 → dn42 空间
...
- 证据：—
- 缺口：依赖 DNL-03 + RTE-05
- 说明：内部 mesh 节点 → 边缘节点（dn42 精确路由或 fallback）→ dn42 隧道；隧道断 → 经 mesh 出口 fallback

## E2E-05 mesh 节点 → 互联网（mesh exit）
...
- 说明：同一 LAN 两个边缘节点公告：一个停机 → 路由引擎切另一个
...
- [ ] E2E-04：mesh 节点 → dn42 + 断链 fallback（待实现）
